...
class TestHelpdeskTicket(common.TransactionCase):
    ...
    def setUpClass(cls):
        ...
        helpdesk_ticket = cls.env["helpdesk.ticket.chain"]
        cls.user_admin = cls.env.ref("base.user_root")
        cls.user_demo = cls.env.ref("base.user_demo")
        cls.stage_closed = cls.env.ref("helpdesk_mgmt_chain.helpdesk_ticket_stage_done")
    ...
    def test_helpdesk_ticket_datetimes(self):
        ...
        self.assertTrue(
            ...
            "Helpdesk Ticket Chain: Helpdesk ticket should "
            "have a last_stage_update at all times.",
        )

        self.assertFalse(
            self.ticket.closed_date,
            "Helpdesk Ticket Chain: No closed date "
        ...
        self.assertTrue(
            self.ticket.closed_date,
            "Helpdesk Ticket Chain: A closed ticket " "should have a closed_date value.",
        )
        self.assertTrue(
            old_stage_update < self.ticket.last_stage_update,
            "Helpdesk Ticket Chain: The last_stage_update "
        ...
        self.assertTrue(
            ...
            "Helpdesk Ticket Chain: An assigned ticket " "should contain a assigned_date.",
        )
    ...
    def test_helpdesk_ticket_number(self):
        self.assertNotEqual(
            self.ticket.number,
            "/",
            "Helpdesk Ticket Chain: A ticket should have " "a number.",
        )
