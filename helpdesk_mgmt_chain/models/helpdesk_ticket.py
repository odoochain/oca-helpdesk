...
class HelpdeskTicket(models.Model):
    _name = "helpdesk.ticket.chain"
    _description = "Helpdesk Ticket Chain"
    _rec_name = "number"
    ...
    def _get_default_stage_id(self):
        return self.env["helpdesk.ticket.chain.stage"].search([], limit=1).id

    @api.model
    def _read_group_stage_ids(self, stages, domain, order):
        stage_ids = self.env["helpdesk.ticket.chain.stage"].search([])
        return stage_ids
    ...
    stage_id = fields.Many2one(
        comodel_name="helpdesk.ticket.chain.stage",
        string="Stage",
    ...
    tag_ids = fields.Many2many(comodel_name="helpdesk.ticket.chain.tag", string="Tags")
    ...
    channel_id = fields.Many2one(
        comodel_name="helpdesk.ticket.chain.channel",
        string="Channel",
        help="Channel indicates where the source of a ticket"
        "comes from (it could be a phone call, an email...)",
    )
    category_id = fields.Many2one(
        comodel_name="helpdesk.ticket.chain.category",
        string="Category",
    )
    team_id = fields.Many2one(
        comodel_name="helpdesk.ticket.chain.team",
        string="Team",
    )
    ...
    attachment_ids = fields.One2many(
        ...
        inverse_name="res_id",
        domain=[("res_model", "=", "helpdesk.ticket.chain")],
    ...
    def write(self, vals):
        for _ticket in self:
            ...
            if vals.get("stage_id"):
                stage = self.env["helpdesk.ticket.chain.stage"].browse([vals["stage_id"]])
    ...
    def _prepare_ticket_number(self, values):
        ...
        return seq.next_by_code("helpdesk.ticket.chain.sequence") or "/"
