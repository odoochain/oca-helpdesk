...
class HelpdeskTicketController(http.Controller):
    ...
    def support_ticket_close(self, **kw):
        ...
        ticket = (
            http.request.env["helpdesk.ticket.chain"]
            .sudo()
    ...
    def create_new_ticket(self, **kw):
        categories = http.request.env["helpdesk.ticket.chain.category"].search(
            [("active", "=", True)]
        )
        email = http.request.env.user.email
        name = http.request.env.user.name
        return http.request.render(
            "helpdesk_mgmt_chain.portal_create_ticket",
            {"categories": categories, "email": email, "name": name},
        )

    @http.route("/submitted/ticket", type="http", auth="user", website=True, csrf=True)
    def submit_ticket(self, **kw):
        category = http.request.env["helpdesk.ticket.chain.category"].browse(
            int(kw.get("category"))
        )
        vals = {
            ...
            "channel_id": request.env["helpdesk.ticket.chain.channel"]
            .sudo()
            .search([("name", "=", "Web")])
            .id,
        ...
        new_ticket = request.env["helpdesk.ticket.chain"].sudo().create(vals)
                    ...
                    request.env["ir.attachment"].sudo().create(
                        {
                            ...
                            "res_model": "helpdesk.ticket.chain",
