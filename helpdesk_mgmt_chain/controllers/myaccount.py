...
class CustomerPortalHelpdesk(CustomerPortal):
    def _prepare_home_portal_values(self, counters):
        ...
        if "ticket_count" in counters:
            helpdesk_model = request.env["helpdesk.ticket.chain"]
            ticket_count = (
    ...
    def portal_my_tickets(
        ...
        HelpdeskTicket = request.env["helpdesk.ticket.chain"]
        ...
        for stage in request.env["helpdesk.ticket.chain.stage"].search([]):
        ...
        return request.render("helpdesk_mgmt_chain.portal_my_tickets", values)
    ...
    def portal_my_ticket(self, ticket_id=None, access_token=None, **kw):
        try:
            ticket_sudo = self._document_check_access(
                "helpdesk.ticket.chain", ticket_id, access_token=access_token
            )
        except (AccessError, MissingError):
            return request.redirect("/my")
        values = self._ticket_get_page_view_values(ticket_sudo, **kw)
        return request.render("helpdesk_mgmt_chain.portal_helpdesk_ticket_page", values)

    def _ticket_get_page_view_values(self, ticket, **kwargs):
        closed_stages = request.env["helpdesk.ticket.chain.stage"].search(
            [("closed", "=", True)]
        )
        values = {
            "page_name": "ticket",
